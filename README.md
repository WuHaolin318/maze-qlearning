...
代码结构清晰、注释详细，适合大一学生作为强化学习入门和科研简历素材。
...
> 《基于 Q-Learning 的智能体迷宫自主导航系统》
> 从零实现 6×6 网格迷宫环境与 Q-learning 智能体，独立设计状态、
> 动作、奖励函数与 epsilon-greedy 探索策略；训练 3000 局后贪心评估
> 成功率 100%，平均 10 步到达终点（随机策略对照仅 50%、平均 120.8 步）；
> 完成学习率、训练轮数、探索策略三组对照实验，并用 matplotlib 输出
> 训练曲线、策略图和路径图；代码含详细中文注释和自动化测试。

## 面试问答要点

1. Q-learning 是什么？
   一种无模型的强化学习算法，用 Q 表估计“状态-动作”的长期回报。
2. Q 更新公式每一部分是什么意思？
   `alpha` 控制更新步长，`r` 是即时奖励，`gamma` 给未来回报打折，
   目标值 `r + gamma * max Q(s',a')` 比旧估计更接近真实值。
3. 探索和利用是什么？
   探索是尝试未知动作来收集信息，利用是选择当前认为最好的动作；
   epsilon-greedy 是两者最简单直接的平衡方式。
4. 怎么证明智能体真的学会了？
   训练后用 epsilon=0 的贪心策略独立评估 100 局，
   成功率和平均步数对比随机策略是客观证据。
5. 它的局限是什么？
   状态空间大时 Q 表存不下，随机环境中更新不稳定；
   这些问题引出 DQN、Double DQN 等后续方法。
